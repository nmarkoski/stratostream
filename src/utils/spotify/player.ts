...
export async function playSpotifyContent({ contextUri, offset, trackUris }: {
    contextUri?: string;
    offset?: number;
    trackUris?: string[];
}) {
    ...
    if (!user.user_metadata?.spotify_premium) {
        ...
        return {
            ...
            description: "The playback of content requires a premium Spotify subscription. If you have a premium subscription, please reauthenticate."
        };
    }
    ...
    if (!user.user_metadata?.spotify_device_id) {
        ...
        return {
            ...
            description: "The device ID for the player has not been set. This is likely due to the player failing to initialize. Please refresh the page or reauthenticate."
        };
    }
    ...
    const device = devices.devices.find(device => device.id === user.user_metadata.spotify_device_id);
    if (!device?.id) return {
        message: "Device ID stale",
        description: "The device ID stored for the player is not valid. This is likely due to the player failing to initialize. Please refresh the page or reauthenticate."
    };
    ...
    try {
        ...
        return {
            ...
            description: "The playback of the selected content has failed for an unknown reason."
        };
    }
}
