...
function BaseGrid({ children, className }: React.HTMLProps<HTMLDivElement>) {
    return (
        <div
            className={cn(
                className,
                "grid gap-3 grid-cols-[repeat(auto-fit,minmax(9rem,max-content))] md:grid-cols-[repeat(auto-fit,minmax(10rem,max-content))]",
            )}
        >
            {children}
        </div>
...
export function SkeletonGrid({
    ...
    return (
        <div
            className={cn(
                "grid gap-3 grid-cols-[repeat(auto-fit,minmax(9rem,1fr))] md:grid-cols-[repeat(auto-fit,minmax(10rem,1fr))]",
                className,
            )}
        >
            {Array(length)
