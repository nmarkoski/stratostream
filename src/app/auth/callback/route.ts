...
export async function GET(request: Request) {
                ...
                try {
                    ...
                    await supabase.auth.updateUser({
                        data: {
                            ...
                            ...(!(user?.user_metadata?.region_preference) ? {region_preference: market} : {})
                        }
                    })

                } catch (error) {
                    console.error('Error checking Spotify status:', error);
                    return failWithMessage('An error occurred while checking Spotify status.');
                }
            } else {
    ...
    async function failWithMessage(message: string) {
        ...
        return NextResponse.redirect(`${origin}/welcome?message=${encodeURIComponent(message)}`);
    }
