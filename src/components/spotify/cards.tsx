...
import {AlbumMenu, ArtistMenu, SimplifiedTrackMenu, TrackMenu} from "@/components/spotify/menus";
...
export function BaseCard({imgData, textData, onClick, className}: {
    imgData: ImageData
    textData: {
        main: string;
        sub?: string;
    ...
    return (
        <Card // hover:shadow-blue-600/15 shadow-xl
            className={cn('flex flex-col justify-center select-none px-3 pt-3 pb-2 hover:ring-2 ring-blue-600/70 transition cursor-pointer', className)}
            onClick={onClick}
        >
...
export function SimplifiedTrackCard({track, index, context, offset, className}: {
            ...
            <Card // hover:shadow-blue-600/15 shadow-xl
                ...
                onClick={async () => {
                    const result = await playSpotifyContent({
                        ...((context && (offset || offset === 0)) && {contextUri: context, offset: offset}),
                        ...((!context || (!offset && offset !== 0)) && {trackUris: [track.uri]})
                    })
                    if(result?.message) {
                        toast({
...
export function TrackCard({track, context, offset, imgSizes, className}: {
    track: Track;
    context?: string;
    offset?: number;
    imgSizes?: ImgSizes;
...
export function AlbumCard({album, imgSizes, className}: {
...
export function ArtistCard({artist, imgSizes, className}: {
    artist: Artist
    imgSizes?: ImgSizes;
} & React.HTMLProps<HTMLDivElement>) {
...
export function SkeletonCard({variant, className}: {
    variant: 1 | 2
} & React.HTMLProps<HTMLDivElement>) {
    ...
    return (
        <Card
            className={cn('flex flex-col justify-center px-3 pt-3 pb-2', className)}
        >
            <Skeleton
                className="w-full h-full aspect-square self-center rounded-md"
            />
