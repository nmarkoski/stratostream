...
function SeekbarLayout() {
    ...
    return (
        <>
            <DesktopFlex className="flex-row justify-center items-center min-h-24 dark:bg-zinc-800/10 shadow-t-md z-20">
                {isActive ? (
                    <SeekbarInfo
                        ...
                        className="flex-grow-0 w-[25%] pl-4 md:pl-6"
                    />
                ) : null}
                <SeekbarControls
                    player={player}
            ...
            <MobileFlex
                className={cn(
                    "h-0 transition-height overflow-hidden bg-zinc-800/10",
                    isClickedMobile ? "h-16" : "",
                )}
            >
                {isActive ? (
                    <SeekbarInfo
                        currentTrack={currentTrack}
                        className="w-full pl-4 pt-2 h-16"
                    />
                ) : null}
            </MobileFlex>
            <MobileFlex className="flex-row justify-center items-center min-h-24 px-4 dark:bg-zinc-800/10 z-20">
                ...
                <div
                    className={cn(
                        "flex-grow-0 flex items-center justify-center transition-all",
                        isActiveMobile ? "ml-3" : "",
                    )}
                ...
                >
                    {isActive ? (
                        <SeekbarImage
                            currentTrack={currentTrack}
                            className={cn(
                                "w-0 rounded-md select-none transition-width",
                                isActiveMobile ? "w-[4.8rem]" : "",
                            )}
                        />
                    ) : null}
                </div>
...
function SeekbarImage({
    ...
    return (
        <HoverCard openDelay={500} onOpenChange={(open) => setIsDimmed(open)}>
            <HoverCardTrigger>
                <SpotifyImage
                    imgData={{
                        ...
                        height:
                            currentTrack.album.images[0].height ?? undefined,
                        loading: "eager",
                    }}
                    className={cn(
                        className,
...
function SeekbarControls({
    ...
    const repeatModeId: {
        ...
        0: {
            icon: <Repeat className="size-full" strokeWidth={2.2} />,
            name: "off",
            next: 1,
        },
        1: {
        ...
        2: {
            icon: <Repeat1 className="size-full" strokeWidth={2.2} />,
            name: "track",
            next: 0,
        },
    };
        ...
        >
            <div className="w-full flex flex-row justify-between md:justify-center items-center md:gap-5 drop-shadow-md">
                <Button
                    ...
                    onClick={() =>
                        changeSpotifyPlaybackSettings({
                            shuffle: !state?.shuffle,
                        })
                    }
                    disabled={!isActive || state?.disallows.toggling_shuffle}
                >
                    <Shuffle className="size-full" strokeWidth={2.2} />
                </Button>
                <Button
                    size="icon"
                    ...
                    disabled={!isActive}
                >
                    <SkipBack className="size-full" strokeWidth={2.2} />
                </Button>
                <Button
                    size="icon"
                    variant="inverse-mobile"
                    className={cn(
                        "rounded-full size-11 p-[0.6rem] bg-zinc-900 text-zinc-50 dark:bg-zinc-100 dark:text-zinc-900 border border-zinc-200/25 ring-blue-700 dark:ring-blue-600 md:hover:ring-2 transition",
                        !isActive || state?.paused ? "" : "ring-2",
                    )}
                    ...
                    {!isActive ? (
                        ...
                        <>
                            ...
                            <Pause
                                className={cn(
                                    "absolute transition-opacity",
                                    state && "paused" in state && state?.paused
                                        ? "opacity-0"
                                        : "",
                                )}
                                strokeWidth={2.5}
                ...
                <Button
                    ...
                    disabled={
                        ...
                        state?.disallows.skipping_next
                    }
                >
                    <SkipForward className="size-full" strokeWidth={2.2} />
                </Button>
                <Button
                    size="icon"
                    variant={
                        state?.repeat_mode ? "default-mobile" : "ghost-mobile"
                    }
                    className="rounded-full size-10 p-[0.65rem] transition"
                    onClick={() =>
                        changeSpotifyPlaybackSettings({
                            repeat: repeatModeId[
                                repeatModeId[state?.repeat_mode ?? 0].next
                            ].name,
                        })
                    }
                    disabled={
                        !isActive || state?.disallows.toggling_repeat_context
                    }
                >
            ...
            <div className="flex flex-row justify-center items-center gap-4 w-full">
                <span className="min-w-6 md:min-w-7 text-center text-xs md:text-sm font-light text-zinc-800 dark:text-zinc-300 select-none">
                    {msToDurationSeek(state?.position ?? 0)}
                </span>
                <Slider
                    defaultValue={[0]}
                ...
                />
                <span className="min-w-6 md:min-w-7 text-center text-xs md:text-sm font-light text-zinc-800 dark:text-zinc-300 select-none">
                    {msToDurationSeek(state?.duration ?? 0)}
                </span>
            </div>
        </div>
    );
