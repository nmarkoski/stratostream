    ...
    return (
        <Button
            ...
            className={cn(
                ...
                isActive &&
                    "font-extrabold bg-zinc-900 dark:bg-zinc-50 text-zinc-50 dark:text-zinc-900 ring-2",
            )}
            asChild
        >
            <Link
                href={
                    searchParam
                        ? `${href}?s=${encodeURIComponent(searchParam)}`
                        : href
                }
                className={cn(isRoot ? "pointer-events-none" : "")}
            >
                {children}
            </Link>
        </Button>
...
export default function HeaderNavBar() {
    return (
        <div className="flex flex-row items-center justify-center md:justify-end px-3 gap-3 md:gap-6 md:tracking-wide w-full">
