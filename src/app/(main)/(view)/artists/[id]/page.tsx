...
export default async function ArtistView({
    ...
    const artist = await spotify.artists.get(params.id).catch((error) => {
        console.error(error);
        return notFound();
    });

    const artistAlbums = await spotify.artists.albums(
        params.id,
        "album,single",
        undefined,
        50,
                ...
                >
                    <div className="flex-1 flex flex-row justify-center items-center p-8 gap-10 md:gap-14 bg-black/25 backdrop-blur-2xl backdrop-saturate-125 backdrop-contrast-125">
                        ...
                        <div className="flex flex-col gap-1 text-zinc-100">
                            ...
                            <span className="text-lg md:text-2xl leading-5 font-base pb-3 md:pb-6 drop-shadow-[0_2px_1.5px_rgba(0,0,0,0.8)]">
                                {artist.followers.total.toLocaleString()}{" "}
                                followers
                            </span>
                            <DialogTrigger asChild>
                                <button className="w-fit text-sm md:text-base leading-5 drop-shadow-[0_2px_1.5px_rgba(0,0,0,0.8)] text-start hover:underline">
                                    Similar artists
                                </button>
                            </DialogTrigger>
                        </div>
