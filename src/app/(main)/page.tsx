...
export default async function Home() {
    ...
    const top50PlaylistID = await getPlaylistIdFromRegion(
        user?.user_metadata.region_preference,
    );

    const top50Playlist = await spotify.playlists.getPlaylistItems(
        top50PlaylistID,
        undefined,
        undefined,
        50,
    ...
    const albumIdChunks = chunkArray(
        [...new Set(topTracks.map((track) => track.album.id))],
        20,
    );
    const artistIdChunks = chunkArray(
        [
            ...new Set(
                topTracks
        ...
        <>
            <div className="p-3 pb-4 md:p-0 md:content-center md:h-[51%]">
                <div className="flex flex-row items-center pl-2 pb-4 md:pl-10 md:pb-4 drop-shadow-md">
                    ...
                    <span className="text-3xl font-bold pl-2">Top tracks</span>
                </div>
                <ItemCarousel
                    items={topTracks}
                    context={`spotify:playlist:${top50PlaylistID}`}
                    slideVariant="full"
                />
            </div>
            <Separator />
